    ...
    async def generate_comprehensive_flowchart(full_context: str, tech_stack: dict) -> Dict[str, Any]:
        ...
        {{
            ...
            "nodes": [
                ...
                {{"id": "frontend", "label": "Frontend App", "type": "component"}},
                {{"id": "api", "label": "API Server", "type": "component"}},
                {{"id": "database", "label": "Database", "type": "storage"}}
            ],
            "edges": [
                {{"from": "user", "to": "frontend", "label": "interacts with"}},
...
# ==============================================================================
# 5. AGENT GRAPH DEFINITION (Unchanged)
# ==============================================================================
def create_agent_graph():
    async def requirements_node(state: AgentState):
        logger.info("--- Running Requirements Expert ---")
        questions = await BlueprintGenerator.generate_clarifying_questions(state["initial_idea"], state["user_profile"])
    ...
    def router(state: AgentState):
        if not state.get("user_answers"): return "Requirements_Expert"
        if not state.get("tech_stack"): return "Architect"
        if not state.get("flowchart"): return "Designer"
        if not state.get("project_structure"): return "Structure_and_Docs_Generator"
        if not state.get("files"): return "Code_Generator"
        return END
...
# ==============================================================================
# 6. FASTAPI APPLICATION (Unchanged)
# ==============================================================================
app = FastAPI(title="AI Blueprint Generator API", version="3.2.0") # Version bump
...
async def generate_blueprint(request: BlueprintRequest):
    ...
    async def stream_generator():
        ...
        try:
            async for chunk in agent_graph.astream(initial_state, {"recursion_limit": 15}):
                ...
                if node_name != "__end__":
                    ...
                    if node_name == "Code_Generator":
                        files_count = len(chunk[node_name].get('files', []))
                        logger.info(f"Code_Generator completed with {files_count} files")
                    ...
                    if not completion_sent:
                        ...
                        has_files = final_state.get('files') and len(final_state.get('files', [])) > 0
                        has_structure = final_state.get('project_structure')
                        has_tech_stack = final_state.get('tech_stack')
                        code_generator_ran = "Code_Generator" in nodes_processed
                        
                        logger.info(f"Completion check: files={has_files}, structure={has_structure}, tech_stack={has_tech_stack}, code_gen={code_generator_ran}")
                        
                        if has_files and has_structure and has_tech_stack and code_generator_ran:
                            completion_payload = {
                                'agent': 'COMPLETE',
                                'output': {
                                    'project': {
                                        'id': str(uuid.uuid4()),
                                        'name': ' '.join(request.initial_idea.split(' ', 2)[:2]) + ' Project',
                            ...
                            logger.warning(f"End reached but missing components or Code_Generator didn't run properly")
                            # Send error completion
                            error_completion = {
                                'agent': 'COMPLETE',
                                'output': {
                                    'error': 'Incomplete blueprint generation',
                                    'project': {},
                                    'files': final_state.get('files', [])
                                },
                                'timestamp': datetime.now().isoformat()
            ...
            error_payload = {"error": "Blueprint generation failed", "details": str(e)}
